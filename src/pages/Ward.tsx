  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-4 text-sm font-medium bg-white p-2 rounded-lg border border-slate-200 shadow-sm">
            <div className="flex items-center gap-2 px-2">
                <div className="w-3 h-3 rounded-full bg-green-500"></div> Available: {available}
            </div>
            <div className="flex items-center gap-2 px-2 border-l border-slate-200">
                <div className="w-3 h-3 rounded-full bg-blue-500"></div> Occupied: {occupied}
            </div>
                <div className="flex items-center gap-2 px-2 border-l border-slate-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {beds.map((bed) => (
          <div key={bed.id} className={`relative p-0 rounded-xl border-2 transition-all overflow-hidden flex flex-col ${
            bed.status === 'Available' ? 'border-green-100 bg-white hover:border-green-300 shadow-sm' :
            bed.status === 'Occupied' ? 'border-blue-100 bg-white hover:border-blue-300 shadow-sm' :
            'border-slate-200 bg-slate-50 hover:border-slate-300'
          }`}>
             {/* Header Card */}
             <div className={`p-4 flex justify-between items-start ${
                 bed.status === 'Available' ? 'bg-green-50/50' :
                 bed.status === 'Occupied' ? 'bg-blue-50/50' : 'bg-slate-100'
             }`}>
                <div>
                    <h3 className="text-lg font-bold text-slate-800">Bed {bed.number}</h3>
                ...
                <span className={`text-xs font-bold px-2 py-1 rounded-full uppercase ${
                     ...
                     bed.status === 'Occupied' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'
                }`}>
                  {bed.status}
                </span>
             </div>
                ...
                {bed.status === 'Occupied' ? (
                   <div className="space-y-4">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center py-4 text-center">
                        <BedDouble size={32} className="text-green-200 mb-2" />
                   ...
                   <>
                      {bed.status === 'Available' && (
                          <button 
                            ...
                            className="w-full py-2 bg-teal-600 text-white rounded-lg text-xs font-bold hover:bg-teal-700 shadow-sm transition flex items-center justify-center gap-2"
                          >
      ...
      {showAdmitModal && selectedBed && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col">
             <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                <div>
                   <h3 className="font-bold text-slate-800 flex items-center gap-2">
                      <BedDouble size={20} className="text-teal-600" />
                      Admit Patient
                   </h3>
             ...
             <div className="p-5 border-t border-slate-200 flex gap-3 bg-slate-50">
                ...
                <button 
                   ...
                   className={`flex-1 py-2.5 rounded-lg font-medium transition shadow-sm flex justify-center items-center gap-2 ${
                       selectedPatientId 
